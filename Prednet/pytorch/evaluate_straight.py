...
def arg_parse():
    ...
    parser.add_argument('--num_plot', default = 8, type = int, metavar = 'N',
...
#     X_test = X_test[:8, ...]                    # to overcome `cuda runtime error: out of memory`
    ...
    X_groundTruth = np.transpose(X_test.detach().cpu().numpy(), (1, 0, 2, 3, 4))      # (timesteps, batch_size, 3, 128, 160)
    X_groundTruth_list = []
...
#     X_test = Variable(torch.from_numpy(X_test).float().cuda())
    ...
    pred = torch.stack(predictions, 2).detach().cpu().numpy()
    pred = np.transpose(pred, (2,0,1,3,4))
    print(X_groundTruth.shape, pred.shape)
    MSE_PredNet  = np.mean((X_groundTruth[1:,...  ] - pred[1:,...])**2)    # look at all timesteps except the first
    MSE_previous = np.mean((X_groundTruth[:-1, ...] - X_groundTruth[1:, ...])**2)
    MSE_previous_pred = np.mean((pred[:-1, ...] - pred[1:, ...])**2)
    ...
    with open(score_file, 'w') as f:
        ...
        f.write("Previous Prediction MSE: %f" % MSE_previous_pred)
...
if __name__ == '__main__':
    ...
    prednet = PredNet(stack_sizes, R_stack_sizes, A_filter_sizes, Ahat_filter_sizes, R_filter_sizes,
                      output_mode = 'prediction', data_format = args.data_format, extrap_start_time=5,return_sequences = True)
    print(prednet)
    prednet.cuda()
