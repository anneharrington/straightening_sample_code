...
#             assert inputs.shape == (0,), f"input {self.conv_layers['i'][lay]}"
...
#                 assert 1==0, f"{Ahat.shape} and {A.shape}"
                E_up   = F.relu(Ahat - A)
        ...
        if self.output_mode in ['R0','R1','R2','R3']:
            output = R_list[int(self.output_mode[1])]
        return output, R_list[2], states
    ...
    def forward(self, A0_withTimeStep, initial_states):
        ...
        for t in range(num_timesteps):
            ...
            output,R2s_out, hidden_states = self.step(A0, hidden_states)
        ...
        if self.output_mode == 'error':
            ...
            return output_list, torch.stack(R2s, 1)
        elif self.output_mode == 'prediction':
            return output_list  # 此时的output_list是timestep个预测帧图像
        elif self.output_mode == 'all':
            pass
        elif self.output_mode in ['R0','R1','R2','R3']:
            return output_list
        else:
