...
#         steps_per_epoch = 500/4
        for step, (frameGroup, target) in enumerate(dataLoader):
            ...
            output,R2s = prednet(batch_frames, states)
            ...
            #get curvature
            curv = computeDistCurv_batch(R2s, grad=True)
            curv_mean = torch.mean(curv,[0,1])
            if curv_mean.item() == float('nan'):
                loss = total_error
            else:
                loss = total_error + curv_mean
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        if sum_trainLoss_in_epoch < min_trainLoss_in_epoch:
            ...
            saveCheckpoint(zcr_state_dict, fileName = os.path.join(args.checkpoint_savePath,'checkpoint_newest.pt' ))


def saveCheckpoint(zcr_state_dict, fileName = 'checkpoint_newest.pt'):
